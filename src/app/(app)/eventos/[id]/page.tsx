...
export default async function AttendancePage({ params, searchParams }: Props) {
  ...
  return (
    <>
      <PageHeading
        eyebrow={fullRoster ? "Chamada" : "Participação"}
        title={event.title}
        description={`${EVENT_TYPE_LABELS[event.type as EventType]} · ${formatDate(event.event_date)}`}
      />
      {!fullRoster && (
        <p className="mb-5 rounded-xl bg-[#edf7f1] p-4 text-sm text-[#365747]">
          Este evento é opcional. “Não participou” não representa falta e não afeta a frequência EBD nem o Radar Pastoral.
        </p>
      )}
      ...
      {!students?.length ? (
        <section className="card p-10 text-center">
          <h2 className="font-bold">Nenhum adolescente ativo</h2>
          <p className="mt-2 text-sm text-[#647268]">Cadastre adolescentes antes de registrar a participação.</p>
        </section>
      ) : (
        <form action={action}>
              ...
              return (
                <fieldset className="card p-4" key={student.id}>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="grid size-11 shrink-0 place-items-center rounded-full bg-[#e3efe7] font-black text-[#176b49]">{initials(student.full_name)}</div>
                    <p className="min-w-0 flex-1 truncate font-bold">{student.preferred_name || student.full_name}</p>
                  </div>
                  <div className={`mt-4 grid gap-2 ${fullRoster ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"}`}>
                    {options.map(([value, label]) => (
                      <label className="cursor-pointer" key={value}>
                        <input className="peer sr-only" type="radio" name={`attendance:${student.id}`} value={value} defaultChecked={defaultStatus === value} required />
                        <span className="flex min-h-12 items-center justify-center rounded-xl border border-[#cbd6cd] px-2 text-center text-xs font-bold peer-checked:border-[#176b49] peer-checked:bg-[#e8f4ec] peer-checked:text-[#176b49]">{label}</span>
                      </label>
                    ))}
                  </div>
                </fieldset>
          ...
          <div className="sticky bottom-4 mt-5 rounded-2xl bg-[#143d2c] p-3 shadow-xl">
            <button className="button-primary w-full bg-[#e6c861] text-[#19382b] hover:bg-[#f0d879]">Salvar participação</button>
          </div>
        </form>
      )}
    </>
